...
while (total_adds < total[0]):  # Website shows x results but, can only x-2 can be called
    ...
    try:
        if ((total[category - 8] - total_category_adds) >= amount_left_u):
            print("total[category-8] = " + str(total[category - 8]) + "\nTotal_category_adds = " + str(total_category_adds))
            amount_left_u = 50
            addon = 0
        else:
            if (adds == 0):
                ...
                if (flag == 2): token = get_token()
                if (flag == 3):  # Definitely cannot call the last remaning question(s)
    ...
    url = 'https://opentdb.com/api.php?amount=50' + "&encode=base64"
    ...
    for o in range(0, len(html_q)):
        if html_q[o] in qa:
            write_log("Repeat Question: " + html_q[o])
            repeats += 1
            continue
        else:
